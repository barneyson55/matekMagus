const TOPIC_ID = 'adatok_abrazolasa';
const TOPIC_NAME = 'Adatok \xe1br\xe1zol\xe1sa';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['frequency'],
  [DIFF_NORMAL]: ['frequency', 'relative'],
  [DIFF_HARD]: ['relative', 'frequency']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'frequency' || kind === 'relative') {
    const total = 10;
    ...
    shuffle(data);
    if (kind === 'frequency') {
      expectedValue = targetCount;
      question = `Adatsor: ${data.join(', ')}. H\xe1ny darab ${target} szerepel?`;
    } else {
      expectedValue = (targetCount / total) * 100;
      tolerance = 0.5;
      answerString = formatNumber(expectedValue, 1);
      question = `Adatsor: ${data.join(', ')}. Mi a(z) ${target} relat\xedv gyakoris\xe1ga sz\xe1zal\xe9kban?`;
    }
  }
