const TOPIC_ID = 'alakzatok_metszespontja';
const TOPIC_NAME = 'K\u00e9t alakzat metsz\u00e9spontja';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['line-vertical-y'],
  [DIFF_NORMAL]: ['line-line-x', 'line-vertical-y'],
  [DIFF_HARD]: ['line-line-y', 'line-line-x']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'line-vertical-y') {
    const m = randomInt(-4, 4);
    const b = randomInt(-6, 6);
    const c = randomInt(-5, 5);
    expectedValue = m * c + b;
    question = 'Az y = ' + m + 'x + ' + b + ' \u00e9s az x = ' + c + ' egyenes metsz\u00e9spontj\u00e1n mi az y koordin\u00e1ta?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'line-line-x') {
    const m1 = randomInt(-4, 4);
        let m2 = randomInt(-4, 4);
        while (m2 === m1) {
          m2 = randomInt(-4, 4);
        }
        const b1 = randomInt(-6, 6);
        const b2 = randomInt(-6, 6);
    expectedValue = (b2 - b1) / (m1 - m2);
    question = 'Adott: y = ' + m1 + 'x + ' + b1 + ' \u00e9s y = ' + m2 + 'x + ' + b2 + '. Mi az x koordin\u00e1ta?';
    answerString = formatNumber(expectedValue, 2);
  }
  else if (kind === 'line-line-y') {
    const m1 = randomInt(-4, 4);
        let m2 = randomInt(-4, 4);
        while (m2 === m1) {
          m2 = randomInt(-4, 4);
        ...
        const b1 = randomInt(-6, 6);
        const b2 = randomInt(-6, 6);
    expectedValue = m1 * ((b2 - b1) / (m1 - m2)) + b1;
    question = 'Adott: y = ' + m1 + 'x + ' + b1 + ' \u00e9s y = ' + m2 + 'x + ' + b2 + '. Mi az y koordin\u00e1ta?';
    answerString = formatNumber(expectedValue, 2);
  }
