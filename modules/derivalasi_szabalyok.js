const TOPIC_ID = 'derivalasi_szabalyok';
const TOPIC_NAME = 'Deriv\u00e1l\u00e1si szab\u00e1lyok';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['const', 'power'],
  [DIFF_NORMAL]: ['power', 'sum'],
  [DIFF_HARD]: ['sum']
};
...
const ruleAInput = document.getElementById('rule-a');
const ruleNInput = document.getElementById('rule-n');
const ruleX0Input = document.getElementById('rule-x0');
const ruleDerivValue = document.getElementById('rule-deriv');
const ruleFormulaValue = document.getElementById('rule-formula');
...
function buildConstQuestion() {
  const c = randomInt(-6, 8);
  const x0 = randomInt(-3, 3);
  return {
    kind: 'const',
    question: `f(x) = ${c}. Mennyi f'(${x0})?`,
    answerString: '0',
    answerType: 'number',
    expectedValue: 0,
    signature: `const:${c}:${x0}`
  };
}
...
function buildPowerQuestion() {
  ...
  return {
    kind: 'power',
    question: `f(x) = ${a}x^${n}. Mennyi f'(${x0})?`,
    answerString: formatNumber(value),
    answerType: 'number',
    expectedValue: value,
    signature: `power:${a}:${n}:${x0}`
  };
}

function buildSumQuestion() {
  const a = pickNonZero(-4, 5);
  const n = randomInt(2, 4);
  const b = pickNonZero(-4, 5);
  const m = randomInt(1, 3);
  const x0 = randomInt(-2, 3);
  const value = a * n * Math.pow(x0, n - 1) + b * m * Math.pow(x0, m - 1);
  return {
    kind: 'sum',
    question: `f(x) = ${a}x^${n} ${b >= 0 ? '+' : '-'} ${Math.abs(b)}x^${m}. Mennyi f'(${x0})?`,
    answerString: formatNumber(value),
    answerType: 'number',
    expectedValue: value,
    signature: `sum:${a}:${n}:${b}:${m}:${x0}`
  };
}

function buildQuestion(kind) {
  if (kind === 'sum') return buildSumQuestion();
  if (kind === 'const') return buildConstQuestion();
  return buildPowerQuestion();
}
...
function updateRuleModel() {
  if (!ruleAInput || !ruleNInput || !ruleX0Input) return;
  const a = Number.parseFloat(ruleAInput.value);
  const n = Number.parseFloat(ruleNInput.value);
  const x0 = Number.parseFloat(ruleX0Input.value);
  if (!Number.isFinite(a) || !Number.isFinite(n) || !Number.isFinite(x0)) {
    if (ruleDerivValue) ruleDerivValue.textContent = '-';
    if (ruleFormulaValue) ruleFormulaValue.textContent = '-';
    if (ruleNote) {
      ruleNote.textContent = 'Adj meg \u00e9rv\u00e9nyes a, n, x0 \u00e9rt\u00e9ket.';
      ruleNote.style.color = '#f04747';
    }
    return;
  }
  const derivValue = a * n * Math.pow(x0, n - 1);
  if (ruleDerivValue) ruleDerivValue.textContent = formatNumber(derivValue);
  if (ruleFormulaValue) ruleFormulaValue.textContent = `${formatNumber(a * n)}\u00b7x^{${n - 1}}`;
  if (ruleNote) {
    ruleNote.textContent = '';
    ruleNote.style.color = '';
  }
}
...
if (ruleAInput) ruleAInput.addEventListener('input', updateRuleModel);
if (ruleNInput) ruleNInput.addEventListener('input', updateRuleModel);
if (ruleX0Input) ruleX0Input.addEventListener('input', updateRuleModel);
