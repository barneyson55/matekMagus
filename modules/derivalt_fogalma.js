const TOPIC_ID = 'derivalt_fogalma';
const TOPIC_NAME = 'A deriv\u00e1lt fogalma';
...
const diffX0Input = document.getElementById('diff-x0');
const diffHInput = document.getElementById('diff-h');
const diffApproxValue = document.getElementById('diff-approx');
const diffExactValue = document.getElementById('diff-exact');
...
function buildLinearQuestion() {
  ...
  return {
    kind: 'linear',
    question: `f(x) = ${a}x${b >= 0 ? '+' : ''}${b}. Mennyi f'(${x0})?`,
    answerString: String(a),
    answerType: 'number',
    expectedValue: a,
    signature: `linear:${a}:${b}:${x0}`
  };
}
...
function buildQuadraticQuestion() {
  ...
  return {
    kind: 'quadratic',
    question: `f(x) = ${a}x^2${b >= 0 ? '+' : ''}${b}x. Mennyi f'(${x0})?`,
    answerString: String(value),
    answerType: 'number',
    expectedValue: value,
    signature: `quadratic:${a}:${b}:${x0}`
  };
}
...
function buildCubicQuestion() {
  ...
  const value = 3 * a * x0 * x0;
  return {
    kind: 'cubic',
    question: `f(x) = ${a}x^3. Mennyi f'(${x0})?`,
    answerString: String(value),
    answerType: 'number',
    expectedValue: value,
    signature: `cubic:${a}:${x0}`
  };
}
...
function updateDiffModel() {
  if (!diffX0Input || !diffHInput) return;
  const x0 = Number.parseFloat(diffX0Input.value);
  const h = Number.parseFloat(diffHInput.value);
  if (!Number.isFinite(x0) || !Number.isFinite(h) || h === 0) {
    if (diffApproxValue) diffApproxValue.textContent = '-';
    if (diffExactValue) diffExactValue.textContent = '-';
    if (diffNote) {
      diffNote.textContent = 'Adj meg \u00e9rv\u00e9nyes x0 \u00e9s nem nulla h \u00e9rt\u00e9ket.';
      diffNote.style.color = '#f04747';
    }
    return;
  }
  const fx0 = x0 * x0;
  const fxh = (x0 + h) * (x0 + h);
  const approx = (fxh - fx0) / h;
  const exact = 2 * x0;
  if (diffApproxValue) diffApproxValue.textContent = formatNumber(approx);
  if (diffExactValue) diffExactValue.textContent = formatNumber(exact);
  if (diffNote) {
    diffNote.textContent = 'Min\u00e9l kisebb h, ann\u00e1l pontosabb.';
    diffNote.style.color = '';
  }
}
...
if (diffX0Input) diffX0Input.addEventListener('input', updateDiffModel);
if (diffHInput) diffHInput.addEventListener('input', updateDiffModel);
