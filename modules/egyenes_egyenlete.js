const TOPIC_ID = 'egyenes_egyenlete';
const TOPIC_NAME = 'Pont, szakasz, egyenes';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['line-value'],
  [DIFF_NORMAL]: ['slope', 'line-value'],
  [DIFF_HARD]: ['intercept', 'slope']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'line-value') {
    ...
    expectedValue = m * x + b;
    question = 'Adott az y = ' + m + 'x + ' + b + ' egyenes. Mennyi y \u00e9rt\u00e9ke, ha x = ' + x + '?';
    ...
    const x1 = randomInt(-6, 6);
        ...
        const x2 = x1 + dx;
        const y2 = y1 + dy;
    expectedValue = (y2 - y1) / (x2 - x1);
    question = 'Add meg az A(' + x1 + ',' + y1 + ') \u00e9s B(' + x2 + ',' + y2 + ') pontokon \u00e1tmen\u0151 egyenes meredeks\u00e9g\u00e9t.';
    ...
    expectedValue = y - m * x;
    question = 'Az egyenes meredeks\u00e9ge m = ' + m + ', \u00e9s \u00e1tmegy a (' + x + ',' + y + ') ponton. Mennyi a b tengelymetszet?';
    answerString = formatNumber(expectedValue, 0);
  }
