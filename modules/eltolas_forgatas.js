const TOPIC_ID = 'eltolas_forgatas';
const TOPIC_NAME = 'Eltol\u00e1s, elforgat\u00e1s';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['translate-x', 'translate-y'],
  [DIFF_NORMAL]: ['rotate-90-x', 'translate-x'],
  [DIFF_HARD]: ['rotate-180-y', 'rotate-270-x']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'translate-x') {
    ...
    expectedValue = x + a;
    question = 'Az A(' + x + ',' + y + ') pontot v=(' + a + ',' + b + ') vektorral eltoljuk. Mi az \u00faj x koordin\u00e1ta?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'translate-y') {
    const x = randomInt(-6, 6);
    const y = randomInt(-6, 6);
    const a = randomInt(-4, 4);
    const b = randomInt(-4, 4);
    expectedValue = y + b;
    question = 'Az A(' + x + ',' + y + ') pontot v=(' + a + ',' + b + ') vektorral eltoljuk. Mi az \u00faj y koordin\u00e1ta?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'rotate-90-x') {
    const x = randomInt(-6, 6);
    const y = randomInt(-6, 6);
    expectedValue = -y;
    question = 'Az A(' + x + ',' + y + ') pontot 90\u00b0-kal balra forgatjuk az orig\u00f3 k\u00f6r\u00fcl. Mi lesz az \u00faj x koordin\u00e1ta?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'rotate-180-y') {
    const x = randomInt(-6, 6);
    const y = randomInt(-6, 6);
    expectedValue = -y;
    question = 'Az A(' + x + ',' + y + ') pontot 180\u00b0-kal elforgatjuk az orig\u00f3 k\u00f6r\u00fcl. Mi lesz az \u00faj y koordin\u00e1ta?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'rotate-270-x') {
    const x = randomInt(-6, 6);
    const y = randomInt(-6, 6);
    expectedValue = y;
    question = 'Az A(' + x + ',' + y + ') pontot 270\u00b0-kal balra forgatjuk az orig\u00f3 k\u00f6r\u00fcl. Mi lesz az \u00faj x koordin\u00e1ta?';
    answerString = formatNumber(expectedValue, 0);
  }
