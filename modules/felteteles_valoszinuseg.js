const TOPIC_ID = 'felteteles_valoszinuseg';
const TOPIC_NAME = 'Felt\xe9teles val\xf3sz\xedn\u0171s\xe9g';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['bag-conditional'],
  [DIFF_NORMAL]: ['bag-conditional', 'dice-conditional'],
  [DIFF_HARD]: ['dice-conditional', 'bag-conditional']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'bag-conditional') {
    const red = randomInt(3, tier === 'hard' ? 8 : 6);
    const blue = randomInt(2, tier === 'hard' ? 6 : 5);
    const givenIsRed = Math.random() < 0.5;
    const targetColor = givenIsRed ? 'k\xe9k' : 'piros';
    const givenColor = givenIsRed ? 'piros' : 'k\xe9k';
    const remainingRed = givenIsRed ? red - 1 : red;
    const remainingBlue = givenIsRed ? blue : blue - 1;
    const favorable = targetColor === 'piros' ? remainingRed : remainingBlue;
    const total = remainingRed + remainingBlue;
    expectedValue = favorable / total;
    answerString = formatFraction(favorable, total);
    question = `Egy urn\xe1ban ${red} piros \xe9s ${blue} k\xe9k goly\xf3 van. K\xe9t goly\xf3t h\xfazunk visszatev\xe9s n\xe9lk\xfcl. Mi a val\xf3sz\xedn\u0171s\xe9ge, hogy a m\xe1sodik ${targetColor}, ha az els\u0151 ${givenColor} volt?`;
  } else if (kind === 'dice-conditional') {
    const scenarios = [
      { given: 'p\xe1rosat', total: 3, target: '4 vagy 6', favorable: 2 },
      { given: '3-n\xe1l nagyobbat', total: 3, target: 'p\xe1rosat', favorable: 2 },
      { given: 'nem 1-et', total: 5, target: 'pr\xedmet', favorable: 2 }
    ];
    const scenario = pickFrom(scenarios);
    expectedValue = scenario.favorable / scenario.total;
    answerString = formatFraction(scenario.favorable, scenario.total);
    question = `Dobunk egy kock\xe1val. Felt\xe9ve, hogy ${scenario.given} dobtunk, mi a val\xf3sz\xedn\u0171s\xe9ge, hogy ${scenario.target}?`;
  }
...
if (practiceStartBtn) {
  practiceStartBtn.addEventListener('click', () => {
    const enabled = getEnabledPracticeDifficulties();
    if (!enabled.length) {
      practiceFeedback.textContent = 'V\xe1lassz legal\xe1bb egy neh\xe9zs\xe9gi szintet.';
      practiceFeedback.style.color = '#f04747';
      return;
    }
    practiceActive = true;
