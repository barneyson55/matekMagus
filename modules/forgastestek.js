const TOPIC_ID = 'forgastestek';
const TOPIC_NAME = 'Forg\u00e1stestek (henger, k\u00fap, g\u00f6mb)';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['cylinder-volume'],
  [DIFF_NORMAL]: ['cone-volume', 'cylinder-volume'],
  [DIFF_HARD]: ['sphere-volume', 'cone-volume']
};
const PI = 3.14;
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'cylinder-volume') {
    const r = randomInt(2, 10);
    const h = randomInt(4, 14);
    expectedValue = PI * r * r * h;
    question = 'Henger sugara r = ' + r + ' cm, magass\u00e1ga h = ' + h + ' cm. Mennyi a t\u00e9rfogata? (\u03c0 \u2248 3,14)';
    answerString = formatNumber(expectedValue, 2);
  }
  else if (kind === 'cone-volume') {
    const r = randomInt(2, 10);
    const h = randomInt(4, 14);
    expectedValue = PI * r * r * h / 3;
    question = 'K\u00fap sugara r = ' + r + ' cm, magass\u00e1ga h = ' + h + ' cm. Mennyi a t\u00e9rfogata? (\u03c0 \u2248 3,14)';
    answerString = formatNumber(expectedValue, 2);
  }
  else if (kind === 'sphere-volume') {
    const r = randomInt(2, 8);
    expectedValue = 4 / 3 * PI * r * r * r;
    question = 'G\u00f6mb sugara r = ' + r + ' cm. Mennyi a t\u00e9rfogata? (\u03c0 \u2248 3,14)';
    answerString = formatNumber(expectedValue, 2);
  }
