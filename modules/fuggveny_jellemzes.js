...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['mono-linear', 'extremum-type-parabola', 'extremum-x-parabola'],
  [DIFF_NORMAL]: ['mono-linear', 'mono-interval-parabola', 'extremum-value-parabola', 'extremum-type-parabola'],
  [DIFF_HARD]: ['mono-abs', 'extremum-type-abs', 'mono-interval-parabola', 'extremum-value-parabola']
};

const TEXT_ALIASES = {
  novekvo: ['novekvo', 'novegvo'],
  csokkeno: ['csokkeno'],
...
const paraExtremum = document.getElementById('para-extremum');
const paraLeft = document.getElementById('para-left');
const paraRight = document.getElementById('para-right');
...
const absExtremum = document.getElementById('abs-extremum');
const absLeft = document.getElementById('abs-left');
const absRight = document.getElementById('abs-right');
...
function formatIntervalLeft(h) {
  return `(-inf, ${formatNumber(h, 3)})`;
}

function formatIntervalRight(h) {
  return `(${formatNumber(h, 3)}, inf)`;
}
...
function pickAbsParams(difficulty) {
  const range = pickParabolaRange(difficulty);
  let a = pickNonZero(-3, 3);
  const h = randomInt(-range, range);
  const k = randomInt(-range, range);
  return { a, h, k };
}
...
function buildExtremumTypeParabola(difficulty) {
  const params = pickParabolaParams(difficulty);
  const extremum = params.a > 0 ? 'minimum' : 'maximum';
  return {
    question: `A f(x) = ${formatParabolaExpression(params.a, params.h, params.k)} parabola minimum vagy maximum? (minimum/maximum)`,
...
function buildMonoIntervalParabola(difficulty) {
  const params = pickParabolaParams(difficulty);
  const side = Math.random() < 0.5 ? 'left' : 'right';
  const isUp = params.a > 0;
  const monotoneLeft = isUp ? 'csokkeno' : 'novekvo';
  const monotoneRight = isUp ? 'novekvo' : 'csokkeno';
  const interval = side === 'left'
    ? formatIntervalLeft(params.h)
...
function buildExtremumTypeAbs(difficulty) {
...
function buildMonoAbsInterval(difficulty) {
  const params = pickAbsParams(difficulty);
  const side = Math.random() < 0.5 ? 'left' : 'right';
  const isUp = params.a > 0;
  const monotoneLeft = isUp ? 'csokkeno' : 'novekvo';
...
function buildQuestion(kind, difficulty) {
  ...
  } else if (kind === 'extremum-type-parabola') {
    result = buildExtremumTypeParabola(difficulty);
  } else if (kind === 'extremum-x-parabola') {
    result = buildExtremumXParabola(difficulty);
  } else if (kind === 'extremum-value-parabola') {
    result = buildExtremumValueParabola(difficulty);
  } else if (kind === 'mono-interval-parabola') {
    result = buildMonoIntervalParabola(difficulty);
  } else if (kind === 'extremum-type-abs') {
    result = buildExtremumTypeAbs(difficulty);
  } else if (kind === 'mono-abs') {
    result = buildMonoAbsInterval(difficulty);
  }
...
function updateParabolaModel() {
  ...
  const isUp = a > 0;
  const leftMonotone = isUp ? 'Cs\u00f6kken\u0151' : 'N\u00f6vekv\u0151';
  const rightMonotone = isUp ? 'N\u00f6vekv\u0151' : 'Cs\u00f6kken\u0151';
  const extremumType = isUp ? 'Minimum' : 'Maximum';

  paraEquation.textContent = `f(x) = ${formatParabolaExpression(a, h, k)}`;
  paraVertex.textContent = `(${formatNumber(h, 3)}, ${formatNumber(k, 3)})`;
  paraExtremum.textContent = `${extremumType}: ${formatNumber(k, 3)}`;
  paraLeft.textContent = `${formatIntervalLeft(h)}: ${leftMonotone}`;
  paraRight.textContent = `${formatIntervalRight(h)}: ${rightMonotone}`;
...
function updateAbsModel() {
  ...
  const intervalLeft = formatIntervalLeft(h);
  ...
  absEquation.textContent = `f(x) = ${formatAbsExpression(a, h, k)}`;
  absExtremum.textContent = `${extremumType}: ${formatNumber(k, 3)}`;
  absLeft.textContent = `${intervalLeft}: ${leftMonotone}`;
  absRight.textContent = `${intervalRight}: ${rightMonotone}`;
  absNote.textContent = '';
  absNote.style.color = '';
}
