const TOPIC_ID = 'fuggvenyvizsgalat';
const TOPIC_NAME = 'F\u00fcggv\u00e9nyvizsg\u00e1lat deriv\u00e1lttal';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['vertex-x'],
  [DIFF_NORMAL]: ['vertex-x', 'vertex-y'],
  [DIFF_HARD]: ['vertex-y', 'extremum-type']
};
...
const quadXvValue = document.getElementById('quad-xv');
const quadYvValue = document.getElementById('quad-yv');
const quadTypeValue = document.getElementById('quad-type');
...
function buildVertexData() {
  ...
  const b = -2 * a * k;
  const c = randomInt(-4, 5);
  const xv = -b / (2 * a);
  const yv = a * xv * xv + b * xv + c;
  return { a, b, c, xv, yv };
}

function buildVertexXQuestion() {
  const { a, b, c, xv } = buildVertexData();
  return {
    kind: 'vertex-x',
    question: `f(x) = ${a}x^2${b >= 0 ? '+' : ''}${b}x${c >= 0 ? '+' : ''}${c}. Mennyi a cs\u00facs x-koordin\u00e1t\u00e1ja?`,
    answerString: formatNumber(xv),
    answerType: 'number',
    expectedValue: xv,
    signature: `vertex-x:${a}:${b}:${c}`
  };
}

function buildVertexYQuestion() {
  const { a, b, c, xv, yv } = buildVertexData();
  return {
    kind: 'vertex-y',
    question: `f(x) = ${a}x^2${b >= 0 ? '+' : ''}${b}x${c >= 0 ? '+' : ''}${c}. Mennyi f(x_v)? (x_v = ${formatNumber(xv)})`,
    answerString: formatNumber(yv),
    answerType: 'number',
    expectedValue: yv,
    signature: `vertex-y:${a}:${b}:${c}`
  };
}

function buildExtremumTypeQuestion() {
  const { a, b, c } = buildVertexData();
  const expectedKey = a > 0 ? 'min' : 'max';
  return {
    kind: 'extremum-type',
    question: `f(x) = ${a}x^2${b >= 0 ? '+' : ''}${b}x${c >= 0 ? '+' : ''}${c}. Minimum vagy maximum?`,
    answerString: a > 0 ? 'minimum' : 'maximum',
    answerType: 'text',
    expectedKey,
    signature: `extremum-type:${a}:${b}:${c}`
  };
}

function buildQuestion(kind) {
  if (kind === 'vertex-y') return buildVertexYQuestion();
  if (kind === 'extremum-type') return buildExtremumTypeQuestion();
  return buildVertexXQuestion();
}
...
function updateQuadraticModel() {
  if (!quadAInput || !quadBInput || !quadCInput) return;
  const a = Number.parseFloat(quadAInput.value);
  const b = Number.parseFloat(quadBInput.value);
  const c = Number.parseFloat(quadCInput.value);
  if (!Number.isFinite(a) || !Number.isFinite(b) || !Number.isFinite(c) || a === 0) {
    if (quadXvValue) quadXvValue.textContent = '-';
    if (quadYvValue) quadYvValue.textContent = '-';
    if (quadTypeValue) quadTypeValue.textContent = '-';
    if (quadNote) {
      quadNote.textContent = 'Adj meg \u00e9rv\u00e9nyes a, b, c \u00e9rt\u00e9keket (a \u2260 0).';
  ...
  const xv = -b / (2 * a);
  const yv = a * xv * xv + b * xv + c;
  if (quadXvValue) quadXvValue.textContent = formatNumber(xv);
  if (quadYvValue) quadYvValue.textContent = formatNumber(yv);
  if (quadTypeValue) quadTypeValue.textContent = a > 0 ? 'Minimum' : 'Maximum';
  if (quadNote) {
