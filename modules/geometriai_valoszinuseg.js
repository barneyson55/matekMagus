const TOPIC_ID = 'geometriai_valoszinuseg';
const TOPIC_NAME = 'Geometriai val\xf3sz\xedn\u0171s\xe9g';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['segment'],
  [DIFF_NORMAL]: ['segment', 'rectangle'],
  [DIFF_HARD]: ['rectangle', 'segment']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'segment') {
    const total = randomInt(10, tier === 'hard' ? 20 : 16);
    const favorable = randomInt(2, Math.max(3, Math.min(8, total - 2)));
    expectedValue = favorable / total;
    answerString = formatFraction(favorable, total);
    question = `Az [0, ${total}] szakaszon v\xe9letlen\xfcl v\xe1lasztunk pontot. Mi a val\xf3sz\xedn\u0171s\xe9ge, hogy [0, ${favorable}] k\xf6z\xe9 esik?`;
  } else if (kind === 'rectangle') {
    const width = randomInt(8, tier === 'hard' ? 16 : 14);
    const height = randomInt(6, tier === 'hard' ? 12 : 10);
    const subW = randomInt(2, Math.max(3, Math.min(8, width - 2)));
    const subH = randomInt(2, Math.max(3, Math.min(8, height - 2)));
    const favorable = subW * subH;
    const total = width * height;
    expectedValue = favorable / total;
    answerString = formatFraction(favorable, total);
    question = `Egy ${width}\xd7${height} t\xe9glalapban v\xe9letlen\xfcl v\xe1lasztunk pontot. Mi a val\xf3sz\xedn\u0171s\xe9ge, hogy a ${subW}\xd7${subH} m\xe9ret\u0171 sarokt\xe9glalapba esik?`;
  }
