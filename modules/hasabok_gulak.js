const TOPIC_ID = 'hasabok_gulak';
const TOPIC_NAME = 'Has\u00e1bok, g\u00fal\u00e1k F, V';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['prism-volume'],
  [DIFF_NORMAL]: ['rectangular-prism-volume', 'prism-volume'],
  [DIFF_HARD]: ['pyramid-volume', 'rectangular-prism-volume']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'prism-volume') {
    const B = randomInt(20, 120);
    const h = randomInt(3, 12);
    expectedValue = B * h;
    question = 'Has\u00e1b alapja B = ' + B + ' cm^2, magass\u00e1ga ' + h + ' cm. Mennyi a t\u00e9rfogata?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'rectangular-prism-volume') {
    const a = randomInt(3, 12);
    const b = randomInt(3, 12);
    const h = randomInt(3, 12);
    expectedValue = a * b * h;
    question = 'T\u00e9glatest oldalai: a = ' + a + ' cm, b = ' + b + ' cm, magass\u00e1g = ' + h + ' cm. Mennyi a t\u00e9rfogata?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'pyramid-volume') {
    const B = randomInt(30, 150);
    const h = randomInt(3, 12);
    expectedValue = B * h / 3;
    question = 'G\u00fala alapja B = ' + B + ' cm^2, magass\u00e1ga ' + h + ' cm. Mennyi a t\u00e9rfogata?';
