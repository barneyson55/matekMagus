const TOPIC_ID = 'hasonlosag';
const TOPIC_NAME = 'K\u00f6z\u00e9ppontos hasonl\u00f3s\u00e1g';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['scale-length'],
  [DIFF_NORMAL]: ['find-scale', 'scale-length'],
  [DIFF_HARD]: ['scale-area', 'find-scale']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'scale-length') {
    const length = randomInt(4, 20);
    const k = randomInt(2, 4);
    expectedValue = length * k;
    question = 'Eredeti hossz ' + length + ' cm, hasonl\u00f3s\u00e1gi ar\u00e1ny k = ' + k + '. Mennyi az \u00faj hossz?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'scale-area') {
    const area = randomInt(10, 80);
    const k = randomInt(2, 4);
    expectedValue = area * k * k;
    question = 'Eredeti ter\u00fclet ' + area + ' cm^2, k = ' + k + '. Mennyi az \u00faj ter\u00fclet?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'find-scale') {
    const original = randomInt(4, 15);
        const k = randomInt(2, 4);
        const scaled = original * k;
    expectedValue = scaled / original;
    question = 'Eredeti hossz ' + original + ' cm, hasonl\u00f3 hossz ' + scaled + ' cm. Mennyi k?';
