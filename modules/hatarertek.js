const TOPIC_ID = 'hatarertek';
const TOPIC_NAME = 'Hat\u00e1r\u00e9rt\u00e9kek, folytonoss\u00e1g';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['removable'],
  [DIFF_NORMAL]: ['removable', 'inf-rational'],
  [DIFF_HARD]: ['inf-rational', 'inf-quadratic', 'removable']
};
...
const infAInput = document.getElementById('inf-a');
const infBInput = document.getElementById('inf-b');
const infCInput = document.getElementById('inf-c');
const infDInput = document.getElementById('inf-d');
const infLimitValue = document.getElementById('inf-limit');
const infNote = document.getElementById('inf-note');

const remAInput = document.getElementById('rem-a');
const remLimitValue = document.getElementById('rem-limit');
...
function buildRemovableQuestion() {
  const a = randomInt(-4, 5);
  const value = 2 * a;
  return {
    kind: 'removable',
    question: `Sz\u00e1m\u00edtsd ki: lim(x\u2192${a}) (x^2-${a * a})/(x-${a}).`,
    answerString: String(value),
    answerType: 'number',
    expectedValue: value,
    signature: `removable:${a}`
  };
}

function buildInfRationalQuestion() {
  const a = pickNonZero(-5, 6);
  ...
  return {
    kind: 'inf-rational',
    question: `Sz\u00e1m\u00edtsd ki: lim(x\u2192\u221e) (${a}x${b >= 0 ? '+' : ''}${b})/(${c}x${d >= 0 ? '+' : ''}${d}).`,
    answerString: formatNumber(limit),
    answerType: 'number',
    expectedValue: limit,
    signature: `inf-rational:${a}:${b}:${c}:${d}`
  };
}

function buildInfQuadraticQuestion() {
  const a = pickNonZero(-4, 5);
  const b = randomInt(-5, 5);
  const c = pickNonZero(-4, 5);
  const d = randomInt(-5, 5);
  const limit = a / c;
  return {
    kind: 'inf-quadratic',
    question: `Sz\u00e1m\u00edtsd ki: lim(x\u2192\u221e) (${a}x^2${b >= 0 ? '+' : ''}${b}x)/(${c}x^2${d >= 0 ? '+' : ''}${d}x).`,
    answerString: formatNumber(limit),
    answerType: 'number',
    expectedValue: limit,
    signature: `inf-quadratic:${a}:${b}:${c}:${d}`
  };
}

function buildQuestion(kind) {
  if (kind === 'inf-rational') return buildInfRationalQuestion();
  if (kind === 'inf-quadratic') return buildInfQuadraticQuestion();
  return buildRemovableQuestion();
}
...
function updateInfModel() {
  if (!infAInput || !infBInput || !infCInput || !infDInput) return;
  const a = Number.parseFloat(infAInput.value);
  const b = Number.parseFloat(infBInput.value);
  const c = Number.parseFloat(infCInput.value);
  const d = Number.parseFloat(infDInput.value);
  if (!Number.isFinite(a) || !Number.isFinite(b) || !Number.isFinite(c) || !Number.isFinite(d) || Math.abs(c) < 1e-8) {
    if (infLimitValue) infLimitValue.textContent = '-';
    if (infNote) {
      infNote.textContent = 'c nem lehet 0.';
      infNote.style.color = '#f04747';
    }
    return;
  }
  const limit = a / c;
  if (infLimitValue) infLimitValue.textContent = formatNumber(limit);
  if (infNote) {
    infNote.textContent = '';
    infNote.style.color = '';
  }
...
function updateRemModel() {
  if (!remAInput) return;
  const a = Number.parseFloat(remAInput.value);
  if (!Number.isFinite(a)) {
    if (remLimitValue) remLimitValue.textContent = '-';
    return;
  }
  if (remLimitValue) remLimitValue.textContent = formatNumber(2 * a);
}
...
if (infAInput) infAInput.addEventListener('input', updateInfModel);
if (infBInput) infBInput.addEventListener('input', updateInfModel);
if (infCInput) infCInput.addEventListener('input', updateInfModel);
if (infDInput) infDInput.addEventListener('input', updateInfModel);
if (remAInput) remAInput.addEventListener('input', updateRemModel);
