const TOPIC_ID = 'hatarozatlan_integral';
const TOPIC_NAME = 'A hat\u00e1rozatlan integr\u00e1l';
...
const intAInput = document.getElementById('int-a');
const intNInput = document.getElementById('int-n');
const intX0Input = document.getElementById('int-x0');
const intValue = document.getElementById('int-value');
const intFormula = document.getElementById('int-formula');
const intNote = document.getElementById('int-note');
...
function buildConstQuestion() {
  ...
  return {
    kind: 'const',
    question: `f(x) = ${c}. Add meg F(${x0})-t, ha F'(x)=f(x) \u00e9s F(0)=0.`,
...
function buildPowerQuestion() {
  ...
  const a = (n + 1) * randomInt(1, 4);
  const x0 = randomInt(1, 3);
  const value = (a / (n + 1)) * Math.pow(x0, n + 1);
  return {
    kind: 'power',
    question: `f(x) = ${a}x^${n}. Add meg F(${x0})-t, ha F'(x)=f(x) \u00e9s F(0)=0.`,
...
function buildQuestion(kind) {
  if (kind === 'power') return buildPowerQuestion();
  return buildConstQuestion();
}
...
function updateIntegralModel() {
  if (!intAInput || !intNInput || !intX0Input) return;
  const a = Number.parseFloat(intAInput.value);
  const n = Number.parseFloat(intNInput.value);
  const x0 = Number.parseFloat(intX0Input.value);
  if (!Number.isFinite(a) || !Number.isFinite(n) || !Number.isFinite(x0) || n === -1) {
    if (intValue) intValue.textContent = '-';
    if (intFormula) intFormula.textContent = '-';
    if (intNote) {
      intNote.textContent = 'Adj meg \u00e9rv\u00e9nyes a, n, x0 \u00e9rt\u00e9ket (n \u2260 -1).';
      intNote.style.color = '#f04747';
    }
    return;
  }
  const coeff = a / (n + 1);
  const value = coeff * Math.pow(x0, n + 1);
  if (intValue) intValue.textContent = formatNumber(value);
  if (intFormula) intFormula.textContent = `${formatNumber(coeff)}\u00b7x^{${n + 1}}`;
  if (intNote) {
    intNote.textContent = 'C = 0-val sz\u00e1molunk.';
    intNote.style.color = '';
  }
}
...
if (intAInput) intAInput.addEventListener('input', updateIntegralModel);
if (intNInput) intNInput.addEventListener('input', updateIntegralModel);
if (intX0Input) intX0Input.addEventListener('input', updateIntegralModel);
