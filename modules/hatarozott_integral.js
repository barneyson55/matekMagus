const TOPIC_ID = 'hatarozott_integral';
const TOPIC_NAME = 'A hat\u00e1rozott integr\u00e1l';
...
const defMInput = document.getElementById('def-m');
const defCInput = document.getElementById('def-c');
const defAInput = document.getElementById('def-a');
const defBInput = document.getElementById('def-b');
const defValue = document.getElementById('def-value');
const defNote = document.getElementById('def-note');
...
function buildLinearQuestion() {
  const m = pickNonZero(-3, 4);
  const c = randomInt(-4, 5);
  const a = randomInt(-1, 1);
  const b = a + randomInt(1, 4);
  const value = (m / 2) * (b * b - a * a) + c * (b - a);
  return {
    kind: 'linear',
    question: `Sz\u00e1m\u00edtsd ki: \u222b_${a}^${b} (${m}x${c >= 0 ? '+' : ''}${c}) dx.`,
    answerString: formatNumber(value),
    answerType: 'number',
    expectedValue: value,
    signature: `linear:${m}:${c}:${a}:${b}`
  };
}

function buildQuadraticQuestion() {
  const k = pickNonZero(1, 3);
  const a = randomInt(0, 2);
  const b = a + randomInt(1, 3);
  const value = (k / 3) * (Math.pow(b, 3) - Math.pow(a, 3));
  return {
    kind: 'quadratic',
    question: `Sz\u00e1m\u00edtsd ki: \u222b_${a}^${b} ${k}x^2 dx.`,
    answerString: formatNumber(value),
    answerType: 'number',
    expectedValue: value,
    signature: `quadratic:${k}:${a}:${b}`
  };
}

function buildQuestion(kind) {
  if (kind === 'quadratic') return buildQuadraticQuestion();
  return buildLinearQuestion();
}
...
function updateDefiniteModel() {
  if (!defMInput || !defCInput || !defAInput || !defBInput) return;
  const m = Number.parseFloat(defMInput.value);
  const c = Number.parseFloat(defCInput.value);
  const a = Number.parseFloat(defAInput.value);
  const b = Number.parseFloat(defBInput.value);
  if (!Number.isFinite(m) || !Number.isFinite(c) || !Number.isFinite(a) || !Number.isFinite(b)) {
    if (defValue) defValue.textContent = '-';
    if (defNote) {
      defNote.textContent = 'Adj meg \u00e9rv\u00e9nyes sz\u00e1mokat.';
      defNote.style.color = '#f04747';
    }
    return;
  }
  const value = (m / 2) * (b * b - a * a) + c * (b - a);
  if (defValue) defValue.textContent = formatNumber(value);
  if (defNote) {
    defNote.textContent = '';
    defNote.style.color = '';
  }
...
if (defMInput) defMInput.addEventListener('input', updateDefiniteModel);
if (defCInput) defCInput.addEventListener('input', updateDefiniteModel);
if (defAInput) defAInput.addEventListener('input', updateDefiniteModel);
if (defBInput) defBInput.addEventListener('input', updateDefiniteModel);
