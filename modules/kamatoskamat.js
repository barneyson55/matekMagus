const TOPIC_ID = 'kamatoskamat';
const TOPIC_NAME = 'Kamatoskamat-sz\u00e1m\u00edt\u00e1s';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['compound-amount'],
  [DIFF_NORMAL]: ['compound-amount', 'compound-interest'],
  [DIFF_HARD]: ['compound-amount', 'compound-interest', 'compound-frequency']
};
...
function compoundAmount(principal, ratePercent, years, freq) {
  ...
  const m = freq || 1;
  return principal * Math.pow(1 + r / m, m * years);
}

function buildCompoundAmountQuestion(withFrequency = false) {
  const principal = randomInt(50, 200) * 1000;
  const rate = randomInt(2, 8);
  const years = randomInt(2, 5);
  const freq = withFrequency ? pickFrom([2, 4, 12]) : 1;
  const amount = compoundAmount(principal, rate, years, freq);
  const expected = roundTo(amount, 2);
  const freqText = withFrequency ? `, m = ${freq}` : '';
  return {
    kind: withFrequency ? 'compound-frequency' : 'compound-amount',
    question: `P = ${principal} Ft, r = ${rate}%, n = ${years}${freqText}. Mennyi a v\u00e9g\u00f6sszeg A? (Kerek\u00edts 2 tizedesre!)`,
    answerString: expected.toFixed(2),
    answerType: 'number',
    expectedValue: expected,
    tolerance: 0.02,
    signature: `compound-amount:${principal}:${rate}:${years}:${freq}`
  };
}

function buildCompoundInterestQuestion() {
  const principal = randomInt(50, 200) * 1000;
  const rate = randomInt(2, 8);
  const years = randomInt(1, 4);
  const amount = compoundAmount(principal, rate, years, 1);
  const interest = roundTo(amount - principal, 2);
  return {
    kind: 'compound-interest',
    question: `P = ${principal} Ft, r = ${rate}%, n = ${years}. Mennyi a kamat \u00f6sszege? (Kerek\u00edts 2 tizedesre!)`,
    answerString: interest.toFixed(2),
    answerType: 'number',
    expectedValue: interest,
    tolerance: 0.02,
    signature: `compound-interest:${principal}:${rate}:${years}`
  };
}

function buildQuestion(kind) {
  if (kind === 'compound-interest') return buildCompoundInterestQuestion();
  if (kind === 'compound-frequency') return buildCompoundAmountQuestion(true);
  return buildCompoundAmountQuestion(false);
}
...
function checkAnswer(answerType, userAnswer, question) {
  ...
  if (answerType === 'number') {
    ...
    return Math.abs(parsed - question.expectedValue) <= tolerance;
  }
  return false;
}
...
function updateInterestModel() {
  if (!principalInput || !rateInput || !yearsInput || !freqInput) return;
  ...
  const freq = Number.parseFloat(freqInput.value);

  if (!Number.isFinite(principal) || !Number.isFinite(rate) || !Number.isFinite(years) || !Number.isFinite(freq) || principal < 0 || rate < 0 || years <= 0 || freq <= 0) {
    if (amountValue) amountValue.textContent = '-';
    if (interestValue) interestValue.textContent = '-';
    if (interestNote) {
      interestNote.textContent = 'Adj meg \u00e9rv\u00e9nyes, pozit\u00edv adatokat.';
      interestNote.style.color = '#f04747';
    }
    return;
  }
  ...
  if (amountValue) amountValue.textContent = formatMoney(amount);
  if (interestValue) interestValue.textContent = formatMoney(interest);
  if (interestNote) {
    interestNote.textContent = 'A kalkul\u00e1tor 2 tizedesre kerek\u00edt.';
    interestNote.style.color = '';
  }
}
...
if (principalInput) principalInput.addEventListener('input', updateInterestModel);
if (rateInput) rateInput.addEventListener('input', updateInterestModel);
if (yearsInput) yearsInput.addEventListener('input', updateInterestModel);
if (freqInput) freqInput.addEventListener('input', updateInterestModel);
