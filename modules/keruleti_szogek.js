const TOPIC_ID = 'keruleti_szogek';
const TOPIC_NAME = 'Ker\u00fcleti \u00e9s k\u00f6z\u00e9pponti sz\u00f6gek';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['inscribed-from-central'],
  [DIFF_NORMAL]: ['central-from-inscribed', 'inscribed-from-arc'],
  [DIFF_HARD]: ['inscribed-from-arc', 'central-from-inscribed']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'inscribed-from-central') {
    const central = randomInt(60, 160);
    expectedValue = central / 2;
    question = 'A k\u00f6z\u00e9pponti sz\u00f6g ' + central + '\u00b0. Mennyi a hozz\u00e1 tartoz\u00f3 ker\u00fcleti sz\u00f6g?';
    answerString = formatNumber(expectedValue, 2);
  }
  else if (kind === 'central-from-inscribed') {
    const inscribed = randomInt(20, 90);
    expectedValue = 2 * inscribed;
    question = 'A ker\u00fcleti sz\u00f6g ' + inscribed + '\u00b0. Mennyi a k\u00f6z\u00e9pponti sz\u00f6g?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'inscribed-from-arc') {
    const arc = randomInt(80, 240);
    expectedValue = arc / 2;
    question = 'Az \u00edv fokm\u00e9rt\u00e9ke ' + arc + '\u00b0. Mennyi a hozz\u00e1 tartoz\u00f3 ker\u00fcleti sz\u00f6g?';
    answerString = formatNumber(expectedValue, 2);
  }
