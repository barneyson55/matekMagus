const TOPIC_ID = 'klasszikus_valoszinuseg';
const TOPIC_NAME = 'Klasszikus val\xf3sz\xedn\u0171s\xe9gi modell';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['dice', 'bag'],
  [DIFF_NORMAL]: ['dice', 'bag', 'coins'],
  [DIFF_HARD]: ['coins', 'dice', 'bag']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'dice') {
    const events = [
      { text: 'p\xe1ros', favorable: 3 },
      { text: 'p\xe1ratlan', favorable: 3 },
      { text: 'legal\xe1bb 5', favorable: 2 },
      { text: 'legfeljebb 3', favorable: 3 },
      { text: 'pr\xedm', favorable: 3 }
    ];
    const event = pickFrom(events);
    const total = 6;
    expectedValue = event.favorable / total;
    answerString = formatFraction(event.favorable, total);
    question = `Dobunk egy szab\xe1lyos kock\xe1val. Mi a val\xf3sz\xedn\u0171s\xe9ge, hogy ${event.text} sz\xe1mot dobunk?`;
  } else if (kind === 'coins') {
    const n = randomInt(2, tier === 'hard' ? 4 : 3);
    ...
    question = `${n} p\xe9nzfeldob\xe1sn\xe1l mi a val\xf3sz\xedn\u0171s\xe9ge, hogy pontosan ${k} fej lesz?`;
  } else if (kind === 'bag') {
    const red = randomInt(3, tier === 'hard' ? 8 : 6);
    const blue = randomInt(2, tier === 'hard' ? 6 : 5);
    const total = red + blue;
    expectedValue = red / total;
    answerString = formatFraction(red, total);
    question = `Egy zs\xe1kban ${red} piros \xe9s ${blue} k\xe9k goly\xf3 van. Mi a val\xf3sz\xedn\u0171s\xe9ge, hogy pirosat h\xfazunk?`;
  }
