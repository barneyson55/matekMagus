const TOPIC_ID = 'kombinaciok';
const TOPIC_NAME = 'Kombin\xe1ci\xf3k';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['combination'],
  [DIFF_NORMAL]: ['combination', 'combination-repeat'],
  [DIFF_HARD]: ['combination', 'combination-repeat']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'combination') {
    const n = randomInt(tier === 'easy' ? 5 : 6, tier === 'hard' ? 12 : 10);
    const k = randomInt(2, Math.min(5, n - 1));
    expectedValue = combination(n, k);
    question = `H\xe1nyf\xe9lek\xe9ppen v\xe1laszthat\xf3 ki ${k} elem ${n} k\xfcl\xf6nb\xf6z\u0151 elem k\xf6z\xfcl (sorrend n\xe9lk\xfcl)?`;
  } else if (kind === 'combination-repeat') {
    const n = randomInt(3, tier === 'hard' ? 7 : 6);
    const k = randomInt(2, tier === 'hard' ? 5 : 4);
    expectedValue = combination(n + k - 1, k);
    question = `H\xe1nyf\xe9lek\xe9ppen v\xe1laszthat\xf3 ki ${k} elem ${n} f\xe9le elemb\u0151l ism\xe9tl\xe9ssel (sorrend n\xe9lk\xfcl)?`;
  }
