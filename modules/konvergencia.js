const TOPIC_ID = 'konvergencia';
const TOPIC_NAME = 'Konvergencia';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['geo-conv'],
  [DIFF_NORMAL]: ['geo-conv', 'geo-limit'],
  [DIFF_HARD]: ['geo-conv', 'geo-limit']
};

const TEXT_ALIASES = {
  conv: ['konvergens', 'osszetarto', 'konvergal'],
  div: ['divergens', 'szettarto', 'nemkonvergens', 'nem konvergens'],
  nlim: ['nemletezik', 'nincs', 'nincs hatertek', 'nincs hatarertek', 'nem letezik']
};
...
const convA1Input = document.getElementById('conv-a1');
const convQInput = document.getElementById('conv-q');
const convStateValue = document.getElementById('conv-state');
const convLimitValue = document.getElementById('conv-limit');
const convNote = document.getElementById('conv-note');
...
function normalizeText(value) {
  return (value || '')
    ...
    .replace(/\u00e9|\u00e8|\u00ea|\u00eb/g, 'e')
    .replace(/\u00ed|\u00ec|\u00ee|\u00ef/g, 'i')
    .replace(/\u00f3|\u00f6|\u0151|\u00f2|\u00f4/g, 'o')
    .replace(/\u00fa|\u00fc|\u0171|\u00f9|\u00fb/g, 'u');
}
...
function resolveTextKey(raw) {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    if (values.some((value) => normalizeText(value) === normalized)) {
      return key;
    }
  }
  return normalized;
...
function classifyGeometric(a1, q) {
  if (!Number.isFinite(q)) return { converges: false, limit: null };
  if (Math.abs(q) < 1) {
    return { converges: true, limit: 0 };
  }
  if (Math.abs(q - 1) < 1e-8) {
    return { converges: true, limit: a1 };
  }
  return { converges: false, limit: null };
}

function buildGeoConvQuestion() {
  const a1 = randomInt(1, 6);
  const q = pickFrom([-2, -1, -0.5, 0.5, 1, 2]);
  const verdict = classifyGeometric(a1, q);
  const expectedKey = verdict.converges ? 'conv' : 'div';
  return {
    kind: 'geo-conv',
    question: `A sorozat a_n = ${a1} \u00b7 ${formatNumber(q)}^{n-1}. Konvergens? (konvergens/divergens)`,
    answerString: verdict.converges ? 'konvergens' : 'divergens',
    answerType: 'text',
    expectedKey,
    signature: `geo-conv:${a1}:${q}`
  };
}

function buildGeoLimitQuestion() {
  const a1 = randomInt(1, 6);
  const q = pickFrom([0.5, -0.5, 1]);
  const verdict = classifyGeometric(a1, q);
  const expected = verdict.limit ?? 0;
  return {
    kind: 'geo-limit',
    question: `Add meg a hat\u00e1r\u00e9rt\u00e9ket: a_n = ${a1} \u00b7 ${formatNumber(q)}^{n-1}.`,
    answerString: formatNumber(expected),
    answerType: 'number',
    expectedValue: expected,
    signature: `geo-limit:${a1}:${q}`
  };
}

function buildQuestion(kind) {
  if (kind === 'geo-limit') return buildGeoLimitQuestion();
  return buildGeoConvQuestion();
}
...
function updateConvergenceModel() {
  if (!convA1Input || !convQInput) return;
  const a1 = Number.parseFloat(convA1Input.value);
  const q = Number.parseFloat(convQInput.value);
  if (!Number.isFinite(a1) || !Number.isFinite(q)) {
    if (convStateValue) convStateValue.textContent = '-';
    if (convLimitValue) convLimitValue.textContent = '-';
    if (convNote) {
      convNote.textContent = 'Adj meg \u00e9rv\u00e9nyes a1 \u00e9s q \u00e9rt\u00e9ket.';
      convNote.style.color = '#f04747';
    }
    return;
  }
  const verdict = classifyGeometric(a1, q);
  if (convStateValue) convStateValue.textContent = verdict.converges ? 'Konvergens' : 'Divergens';
  if (convLimitValue) convLimitValue.textContent = verdict.converges ? formatNumber(verdict.limit) : 'Nincs';
  if (convNote) {
    convNote.textContent = verdict.converges ? 'A sorozat hat\u00e1r\u00e9rt\u00e9ke l\u00e9tezik.' : 'A sorozat nem tart egy \u00e9rt\u00e9khez.';
    convNote.style.color = '';
  }
}
...
if (convA1Input) convA1Input.addEventListener('input', updateConvergenceModel);
if (convQInput) convQInput.addEventListener('input', updateConvergenceModel);
