const TOPIC_ID = 'kor_egyenlete';
const TOPIC_NAME = 'K\u00f6r egyenlete';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['radius-squared'],
  [DIFF_NORMAL]: ['radius-from-r2', 'radius-squared'],
  [DIFF_HARD]: ['distance-squared', 'radius-from-r2']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'radius-squared') {
    const r = randomInt(2, 12);
    expectedValue = r * r;
    question = 'Egy k\u00f6r sugara r = ' + r + ' cm. Mennyi r^2?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'radius-from-r2') {
    const r = randomInt(3, 12);
        const r2 = r * r;
    expectedValue = Math.sqrt(r2);
    question = 'A k\u00f6r egyenlete x^2 + y^2 = ' + r2 + '. Mennyi a sug\u00e1r?';
    answerString = formatNumber(expectedValue, 2);
  }
  else if (kind === 'distance-squared') {
    const h = randomInt(-6, 6);
    const k = randomInt(-6, 6);
    const x = randomInt(-6, 6);
    const y = randomInt(-6, 6);
    expectedValue = (x - h) * (x - h) + (y - k) * (y - k);
    question = 'K\u00f6z\u00e9ppont C(' + h + ',' + k + '), pont P(' + x + ',' + y + '). Mennyi a CP t\u00e1vols\u00e1g n\u00e9gyzete?';
    answerString = formatNumber(expectedValue, 0);
  }
