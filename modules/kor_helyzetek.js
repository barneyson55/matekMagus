const TOPIC_ID = 'kor_helyzetek';
const TOPIC_NAME = 'K\u00f6r \u00e9s egyenes, k\u00f6r \u00e9s k\u00f6r';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['line-circle'],
  [DIFF_NORMAL]: ['line-circle', 'circle-circle'],
  [DIFF_HARD]: ['circle-circle']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'line-circle') {
    const r = randomInt(4, 10);
        const deltaOptions = [-3, -1, 0, 2, 4];
        const delta = deltaOptions[randomInt(0, deltaOptions.length - 1)];
        const d = Math.max(0, r + delta);
    expectedValue = d < r ? 2 : d === r ? 1 : 0;
    question = 'Adott egy k\u00f6r r = ' + r + ' cm sug\u00e1rral. Az egyenes t\u00e1vols\u00e1ga a k\u00f6z\u00e9ppontt\u00f3l d = ' + d + ' cm. H\u00e1ny metsz\u00e9spont van?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'circle-circle') {
    const r1 = randomInt(3, 9);
        const r2 = randomInt(2, 8);
        const caseType = randomInt(0, 2);
        let d = 0;
        if (caseType === 0) {
          d = r1 + r2 + randomInt(1, 3);
        } else if (caseType === 1) {
          d = r1 + r2;
        } else {
          const minD = Math.abs(r1 - r2) + 1;
          const maxD = r1 + r2 - 1;
          d = randomInt(minD, maxD);
        }
    expectedValue = d > r1 + r2 ? 0 : d === r1 + r2 ? 1 : d < Math.abs(r1 - r2) ? 0 : d === Math.abs(r1 - r2) ? 1 : 2;
    question = 'K\u00e9t k\u00f6r sugara r1 = ' + r1 + ' cm \u00e9s r2 = ' + r2 + ' cm, a k\u00f6z\u00e9ppontok t\u00e1vols\u00e1ga d = ' + d + ' cm. H\u00e1ny metsz\u00e9spontjuk van?';
    answerString = formatNumber(expectedValue, 0);
  }
