const TOPIC_ID = 'kozepertekek';
const TOPIC_NAME = 'K\xf6z\xe9p\xe9rt\xe9kek';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['mean', 'median'],
  [DIFF_NORMAL]: ['mean', 'median', 'mode'],
  [DIFF_HARD]: ['mean', 'median', 'mode']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'mean') {
    const count = tier === 'hard' ? 6 : 5;
    const values = [];
    let sum = 0;
    for (let i = 0; i < count - 1; i += 1) {
      const v = randomInt(4, 16);
      values.push(v);
      sum += v;
    }
    let last = randomInt(4, 16);
    if (tier !== 'hard') {
      let attempts = 0;
      while (attempts < 20) {
        const targetMean = randomInt(6, 12);
        const candidate = targetMean * count - sum;
    ...
    values.push(last);
    sum += last;
    expectedValue = sum / count;
    if (!Number.isInteger(expectedValue)) {
      tolerance = 0.05;
      answerString = formatNumber(expectedValue, 2);
    }
    question = `Sz\xe1mok: ${values.join(', ')}. Mennyi az \xe1tlag?`;
  } else if (kind === 'median') {
    const count = tier === 'hard' ? 6 : 5;
    const values = Array.from({ length: count }, () => randomInt(3, 18));
    const sorted = [...values].sort((a, b) => a - b);
    if (count % 2 === 1) {
      expectedValue = sorted[Math.floor(count / 2)];
    } else {
      expectedValue = (sorted[count / 2 - 1] + sorted[count / 2]) / 2;
      tolerance = 0.05;
      answerString = formatNumber(expectedValue, 2);
    }
    question = `Sz\xe1mok: ${values.join(', ')}. Mennyi a medi\xe1n?`;
  } else if (kind === 'mode') {
    const base = randomInt(4, 12);
    const values = [base, base, base];
    const used = new Set([base]);
    while (values.length < 7) {
      const v = randomInt(3, 15);
    ...
    shuffle(values);
    expectedValue = base;
    question = `Sz\xe1mok: ${values.join(', ')}. Mennyi a m\xf3dusz (leggyakoribb \xe9rt\xe9k)?`;
  }
