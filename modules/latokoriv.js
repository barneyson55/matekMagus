const TOPIC_ID = 'latokoriv';
const TOPIC_NAME = 'L\u00e1t\u00f3k\u00f6r\u00edv';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['arc-from-central', 'arc-from-inscribed'],
  [DIFF_NORMAL]: ['arc-from-inscribed', 'inscribed-from-arc'],
  [DIFF_HARD]: ['inscribed-from-arc']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'arc-from-central') {
    const central = randomInt(60, 200);
    expectedValue = central;
    question = 'A k\u00f6z\u00e9pponti sz\u00f6g ' + central + '\u00b0. Mennyi az \u00edv fokm\u00e9rt\u00e9ke?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'arc-from-inscribed') {
    const angle = randomInt(20, 80);
    expectedValue = 2 * angle;
    question = 'A l\u00e1t\u00f3k\u00f6r\u00edvhez tartoz\u00f3 ker\u00fcleti sz\u00f6g ' + angle + '\u00b0. Mennyi az \u00edv fokm\u00e9rt\u00e9ke?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'inscribed-from-arc') {
    const arc = randomInt(60, 200);
    expectedValue = arc / 2;
    question = 'Az \u00edv fokm\u00e9rt\u00e9ke ' + arc + '\u00b0. Mennyi a ker\u00fcleti sz\u00f6g?';
