const TOPIC_ID = 'newton_leibniz';
const TOPIC_NAME = 'Newton\u2013Leibniz formula';
...
const nlAInput = document.getElementById('nl-a');
const nlBInput = document.getElementById('nl-b');
const nlCInput = document.getElementById('nl-c');
const nlX1Input = document.getElementById('nl-x1');
const nlX2Input = document.getElementById('nl-x2');
const nlValue = document.getElementById('nl-value');
...
function buildQuadQuestion() {
  ...
  const value = F(x2) - F(x1);
  return {
    kind: 'quad',
    question: `F(x) = ${a}x^2${b >= 0 ? '+' : ''}${b}x${c >= 0 ? '+' : ''}${c}. Sz\u00e1m\u00edtsd ki F(${x2})-F(${x1}).`,
    answerString: formatNumber(value),
    answerType: 'number',
    expectedValue: value,
    signature: `quad:${a}:${b}:${c}:${x1}:${x2}`
  };
}

function buildCubicQuestion() {
  const a = pickNonZero(-2, 3);
  const b = randomInt(-3, 3);
  const x1 = randomInt(0, 2);
  const x2 = x1 + randomInt(1, 3);
  const F = (x) => a * x * x * x + b * x * x;
  const value = F(x2) - F(x1);
  return {
    kind: 'cubic',
    question: `F(x) = ${a}x^3${b >= 0 ? '+' : ''}${b}x^2. Sz\u00e1m\u00edtsd ki F(${x2})-F(${x1}).`,
    answerString: formatNumber(value),
    answerType: 'number',
    expectedValue: value,
    signature: `cubic:${a}:${b}:${x1}:${x2}`
  };
}

function buildQuestion(kind) {
  if (kind === 'cubic') return buildCubicQuestion();
  return buildQuadQuestion();
}
...
function updateNewtonModel() {
  if (!nlAInput || !nlBInput || !nlCInput || !nlX1Input || !nlX2Input) return;
  const a = Number.parseFloat(nlAInput.value);
  const b = Number.parseFloat(nlBInput.value);
  const c = Number.parseFloat(nlCInput.value);
  const x1 = Number.parseFloat(nlX1Input.value);
  const x2 = Number.parseFloat(nlX2Input.value);
  if (!Number.isFinite(a) || !Number.isFinite(b) || !Number.isFinite(c) || !Number.isFinite(x1) || !Number.isFinite(x2)) {
    if (nlValue) nlValue.textContent = '-';
    if (nlNote) {
      nlNote.textContent = 'Adj meg \u00e9rv\u00e9nyes sz\u00e1mokat.';
      nlNote.style.color = '#f04747';
    }
    return;
  }
  const F = (x) => a * x * x + b * x + c;
  const value = F(x2) - F(x1);
  if (nlValue) nlValue.textContent = formatNumber(value);
  if (nlNote) {
    nlNote.textContent = 'F(x) = a x^2 + b x + c';
    nlNote.style.color = '';
  }
}
...
if (nlAInput) nlAInput.addEventListener('input', updateNewtonModel);
if (nlBInput) nlBInput.addEventListener('input', updateNewtonModel);
if (nlCInput) nlCInput.addEventListener('input', updateNewtonModel);
if (nlX1Input) nlX1Input.addEventListener('input', updateNewtonModel);
if (nlX2Input) nlX2Input.addEventListener('input', updateNewtonModel);
