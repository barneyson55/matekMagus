const TOPIC_ID = 'permutaciok';
const TOPIC_NAME = 'Permut\xe1ci\xf3k';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['factorial', 'distinct'],
  [DIFF_NORMAL]: ['distinct', 'repeat'],
  [DIFF_HARD]: ['repeat', 'factorial', 'distinct']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'factorial') {
    const n = randomInt(tier === 'easy' ? 3 : 4, tier === 'hard' ? 8 : 7);
    expectedValue = factorial(n);
    question = `Sz\xe1mold ki: ${n}!`;
  } else if (kind === 'distinct') {
    const n = randomInt(tier === 'easy' ? 4 : 5, tier === 'hard' ? 9 : 8);
    expectedValue = factorial(n);
    question = `H\xe1nyf\xe9lek\xe9ppen rendezhet\u0151 ${n} k\xfcl\xf6nb\xf6z\u0151 elem?`;
  } else if (kind === 'repeat') {
    let total = 0;
    let first = 0;
    let second = 0;
    let tries = 0;
    while (tries < 20) {
      total = randomInt(tier === 'hard' ? 8 : 7, tier === 'hard' ? 10 : 9);
      first = randomInt(2, 3);
      second = randomInt(2, 3);
      if (total - first - second >= 1) break;
      tries += 1;
    }
    const denom = factorial(first) * factorial(second);
    expectedValue = factorial(total) / denom;
    question = `Egy ${total} bet\u0171s sz\xf3ban ${first} bet\u0171 egyforma \xe9s m\xe9g ${second} bet\u0171 egyforma. H\xe1ny k\xfcl\xf6nb\xf6z\u0151 permut\xe1ci\xf3 van?`;
  }
