const TOPIC_ID = 'specialis_negyszogek';
const TOPIC_NAME = 'H\u00farn\u00e9gysz\u00f6gek, \u00e9rint\u0151n\u00e9gysz\u00f6gek';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['cyclic-opposite'],
  [DIFF_NORMAL]: ['cyclic-opposite', 'tangent-missing-side'],
  [DIFF_HARD]: ['tangent-missing-side', 'cyclic-opposite']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'cyclic-opposite') {
    const angle = randomInt(40, 140);
    expectedValue = 180 - angle;
    question = 'Egy h\u00farn\u00e9gysz\u00f6gben az egyik sz\u00f6g ' + angle + '\u00b0. Mennyi a szemk\u00f6zti sz\u00f6g?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'tangent-missing-side') {
    const a = randomInt(6, 14);
    const b = randomInt(3, 10);
    const c = randomInt(6, 14);
    expectedValue = a + c - b;
    question = '\u00c9rint\u0151n\u00e9gysz\u00f6gben a = ' + a + ' cm, b = ' + b + ' cm, c = ' + c + ' cm. Mennyi a d oldal hossza, ha a + c = b + d?';
    answerString = formatNumber(expectedValue, 0);
  }
