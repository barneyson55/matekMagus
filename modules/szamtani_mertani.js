const TOPIC_ID = 'szamtani_mertani';
const TOPIC_NAME = 'Sz\u00e1mtani \u00e9s m\u00e9rtani sorozatok';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['arith-nth', 'geom-nth'],
  [DIFF_NORMAL]: ['arith-nth', 'geom-nth', 'arith-sum'],
  [DIFF_HARD]: ['arith-nth', 'geom-nth', 'arith-sum', 'geom-sum']
};
...
const arithA1Input = document.getElementById('arith-a1');
const arithDInput = document.getElementById('arith-d');
const arithNInput = document.getElementById('arith-n');
const arithAnValue = document.getElementById('arith-an');
const arithSnValue = document.getElementById('arith-sn');
const arithNote = document.getElementById('arith-note');

const geomA1Input = document.getElementById('geom-a1');
const geomQInput = document.getElementById('geom-q');
const geomNInput = document.getElementById('geom-n');
const geomAnValue = document.getElementById('geom-an');
const geomSnValue = document.getElementById('geom-sn');
...
function buildArithNthQuestion() {
  const a1 = randomInt(1, 12);
  const d = pickNonZero(-5, 6);
  ...
  return {
    ...
    answerType: 'number',
    expectedValue: value,
    signature: `arith-nth:${a1}:${d}:${n}`
  };
}

function buildArithSumQuestion() {
  const a1 = randomInt(1, 10);
  const d = pickNonZero(-4, 5);
  const n = pickEven(4, 10);
  const sum = (n / 2) * (2 * a1 + (n - 1) * d);
  return {
    kind: 'arith-sum',
    question: `Sz\u00e1mtani sorozatban a1 = ${a1}, d = ${d}. Mennyi az els\u0151 ${n} tag \u00f6sszege?`,
    answerString: String(sum),
    answerType: 'number',
    expectedValue: sum,
    signature: `arith-sum:${a1}:${d}:${n}`
  };
}

function buildGeomNthQuestion() {
  const a1 = randomInt(1, 6);
  const q = pickFrom([2, 3, 4]);
  const n = randomInt(3, 6);
  const value = a1 * Math.pow(q, n - 1);
  return {
    kind: 'geom-nth',
    question: `M\u00e9rtani sorozatban a1 = ${a1}, q = ${q}. Mennyi a a${n}?`,
    answerString: String(value),
    answerType: 'number',
    expectedValue: value,
    signature: `geom-nth:${a1}:${q}:${n}`
  };
}

function buildGeomSumQuestion() {
  const a1 = randomInt(1, 4);
  const q = pickFrom([2, 3]);
  const n = randomInt(3, 6);
  const sum = a1 * (Math.pow(q, n) - 1) / (q - 1);
  return {
    kind: 'geom-sum',
    question: `M\u00e9rtani sorozatban a1 = ${a1}, q = ${q}. Mennyi az els\u0151 ${n} tag \u00f6sszege?`,
    answerString: String(sum),
    answerType: 'number',
    expectedValue: sum,
    signature: `geom-sum:${a1}:${q}:${n}`
  };
}

function buildQuestion(kind) {
  if (kind === 'arith-nth') return buildArithNthQuestion();
  if (kind === 'arith-sum') return buildArithSumQuestion();
  if (kind === 'geom-sum') return buildGeomSumQuestion();
  return buildGeomNthQuestion();
}
...
function updateArithmeticModel() {
  if (!arithA1Input || !arithDInput || !arithNInput) return;
  const a1 = Number.parseFloat(arithA1Input.value);
  const d = Number.parseFloat(arithDInput.value);
  const n = Number.parseFloat(arithNInput.value);
  if (!Number.isFinite(a1) || !Number.isFinite(d) || !Number.isFinite(n) || n <= 0) {
    if (arithAnValue) arithAnValue.textContent = '-';
    if (arithSnValue) arithSnValue.textContent = '-';
    if (arithNote) {
      arithNote.textContent = 'Adj meg \u00e9rv\u00e9nyes a1, d \u00e9s n \u00e9rt\u00e9ket.';
      arithNote.style.color = '#f04747';
    }
    return;
  }
  const an = a1 + (n - 1) * d;
  const sn = (n / 2) * (2 * a1 + (n - 1) * d);
  if (arithAnValue) arithAnValue.textContent = formatNumber(an);
  if (arithSnValue) arithSnValue.textContent = formatNumber(sn);
  if (arithNote) {
    arithNote.textContent = '';
    arithNote.style.color = '';
  }
}

function updateGeometricModel() {
  if (!geomA1Input || !geomQInput || !geomNInput) return;
  const a1 = Number.parseFloat(geomA1Input.value);
  const q = Number.parseFloat(geomQInput.value);
  const n = Number.parseFloat(geomNInput.value);
  if (!Number.isFinite(a1) || !Number.isFinite(q) || !Number.isFinite(n) || n <= 0) {
    if (geomAnValue) geomAnValue.textContent = '-';
    if (geomSnValue) geomSnValue.textContent = '-';
    if (geomNote) {
      geomNote.textContent = 'Adj meg \u00e9rv\u00e9nyes a1, q \u00e9s n \u00e9rt\u00e9ket.';
      geomNote.style.color = '#f04747';
    }
    return;
  }
  ...
  if (geomAnValue) geomAnValue.textContent = formatNumber(an);
  if (geomSnValue) geomSnValue.textContent = formatNumber(sn);
  if (geomNote) {
    geomNote.textContent = '';
    geomNote.style.color = '';
  }
}
...
if (arithA1Input) arithA1Input.addEventListener('input', updateArithmeticModel);
if (arithDInput) arithDInput.addEventListener('input', updateArithmeticModel);
if (arithNInput) arithNInput.addEventListener('input', updateArithmeticModel);
if (geomA1Input) geomA1Input.addEventListener('input', updateGeometricModel);
if (geomQInput) geomQInput.addEventListener('input', updateGeometricModel);
if (geomNInput) geomNInput.addEventListener('input', updateGeometricModel);
...
updateArithmeticModel();
updateGeometricModel();
