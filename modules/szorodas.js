const TOPIC_ID = 'szorodas';
const TOPIC_NAME = 'Sz\xf3r\xf3d\xe1si mutat\xf3k';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['range'],
  [DIFF_NORMAL]: ['range', 'variance'],
  [DIFF_HARD]: ['variance', 'stdev']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'range') {
    ...
    expectedValue = max - min;
    question = `Sz\xe1mok: ${values.join(', ')}. Mennyi a terjedelem?`;
  } else if (kind === 'variance') {
    const count = tier === 'hard' ? 6 : 5;
    const values = Array.from({ length: count }, () => randomInt(3, 14));
    expectedValue = variance(values);
    tolerance = 0.05;
    answerString = formatNumber(expectedValue, 2);
    question = `Sz\xe1mok: ${values.join(', ')}. Mennyi a variancia (n-nel osztva)?`;
  } else if (kind === 'stdev') {
    const count = tier === 'hard' ? 6 : 5;
    const values = Array.from({ length: count }, () => randomInt(3, 14));
    const varValue = variance(values);
    expectedValue = Math.sqrt(varValue);
    tolerance = 0.05;
    answerString = formatNumber(expectedValue, 2);
    question = `Sz\xe1mok: ${values.join(', ')}. Mennyi a sz\xf3r\xe1s (n-nel osztva)?`;
  }
