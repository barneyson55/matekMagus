const TOPIC_ID = 'terulet_kerulet';
const TOPIC_NAME = 'Ter\u00fclet- \u00e9s ker\u00fcletsz\u00e1m\u00edt\u00e1s';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['rect-perimeter', 'rect-area'],
  [DIFF_NORMAL]: ['triangle-area', 'parallelogram-area', 'rect-area'],
  [DIFF_HARD]: ['circle-area', 'circle-perimeter', 'triangle-area']
};
const PI = 3.14;
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'rect-perimeter') {
    const a = randomInt(3, 18);
    const b = randomInt(3, 18);
    expectedValue = 2 * (a + b);
    question = 'T\u00e9glalap oldalai: ' + a + ' cm \u00e9s ' + b + ' cm. Mennyi a ker\u00fclete?';
    ...
    const b = randomInt(3, 18);
    expectedValue = a * b;
    question = 'T\u00e9glalap oldalai: ' + a + ' cm \u00e9s ' + b + ' cm. Mennyi a ter\u00fclete?';
    answerString = formatNumber(expectedValue, 0);
  }
    ...
    expectedValue = b * h / 2;
    question = 'H\u00e1romsz\u00f6g alapja ' + b + ' cm, magass\u00e1ga ' + h + ' cm. Mennyi a ter\u00fclete?';
    answerString = formatNumber(expectedValue, 2);
  }
  else if (kind === 'parallelogram-area') {
    const a = randomInt(4, 18);
    const h = randomInt(3, 16);
    expectedValue = a * h;
    question = 'Paralelogramma alapja ' + a + ' cm, magass\u00e1ga ' + h + ' cm. Mennyi a ter\u00fclete?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'circle-perimeter') {
    const r = randomInt(2, 12);
    expectedValue = 2 * PI * r;
    question = 'K\u00f6r sugara ' + r + ' cm. Mennyi a ker\u00fclete? (\u03c0 \u2248 3,14)';
    answerString = formatNumber(expectedValue, 2);
  }
  else if (kind === 'circle-area') {
    const r = randomInt(2, 12);
    expectedValue = PI * r * r;
    question = 'K\u00f6r sugara ' + r + ' cm. Mennyi a ter\u00fclete? (\u03c0 \u2248 3,14)';
    answerString = formatNumber(expectedValue, 2);
  }
