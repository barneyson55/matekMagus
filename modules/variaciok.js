const TOPIC_ID = 'variaciok';
const TOPIC_NAME = 'Vari\xe1ci\xf3k';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['variation', 'variation-repeat'],
  [DIFF_NORMAL]: ['variation', 'variation-repeat'],
  [DIFF_HARD]: ['variation', 'variation-repeat']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'variation') {
    const n = randomInt(tier === 'easy' ? 5 : 6, tier === 'hard' ? 10 : 9);
    const k = randomInt(2, Math.min(4, n - 1));
    expectedValue = variation(n, k);
    question = `H\xe1nyf\xe9lek\xe9ppen v\xe1laszthat\xf3 ki sorrendben ${k} elem ${n} k\xfcl\xf6nb\xf6z\u0151 elem k\xf6z\xfcl?`;
  } else if (kind === 'variation-repeat') {
    const n = randomInt(3, tier === 'hard' ? 7 : 6);
    const k = randomInt(2, tier === 'hard' ? 5 : 4);
    expectedValue = Math.pow(n, k);
    question = `H\xe1nyf\xe9lek\xe9ppen v\xe1laszthat\xf3 ki sorrendben ${k} elem ${n} elemb\u0151l, ha ism\xe9tl\xe9s megengedett?`;
  }
