const TOPIC_ID = 'vektorok';
const TOPIC_NAME = 'Vektorok (skal\u00e1ris szorzat)';
...
const QUESTION_TYPES_BY_DIFFICULTY = {
  [DIFF_EASY]: ['vector-sum-x', 'vector-sum-y'],
  [DIFF_NORMAL]: ['dot-product', 'vector-sum-x'],
  [DIFF_HARD]: ['length-squared', 'dot-product']
};
...
function buildQuestion(kind, difficulty) {
  ...
  if (kind === 'vector-sum-x') {
    const a = randomInt(-6, 6);
    const b = randomInt(-6, 6);
    const c = randomInt(-6, 6);
    const d = randomInt(-6, 6);
    expectedValue = a + c;
    question = 'v = (' + a + ',' + b + '), w = (' + c + ',' + d + '). Mennyi a v + w x-koordin\u00e1t\u00e1ja?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'vector-sum-y') {
    const a = randomInt(-6, 6);
    const b = randomInt(-6, 6);
    const c = randomInt(-6, 6);
    const d = randomInt(-6, 6);
    expectedValue = b + d;
    question = 'v = (' + a + ',' + b + '), w = (' + c + ',' + d + '). Mennyi a v + w y-koordin\u00e1t\u00e1ja?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'dot-product') {
    const a = randomInt(-5, 5);
    const b = randomInt(-5, 5);
    const c = randomInt(-5, 5);
    const d = randomInt(-5, 5);
    expectedValue = a * c + b * d;
    question = 'v = (' + a + ',' + b + '), w = (' + c + ',' + d + '). Mennyi a skal\u00e1ris szorzat v \u00b7 w?';
    answerString = formatNumber(expectedValue, 0);
  }
  else if (kind === 'length-squared') {
    const a = randomInt(-6, 6);
    const b = randomInt(-6, 6);
    expectedValue = a * a + b * b;
    question = 'v = (' + a + ',' + b + '). Mennyi a |v|^2?';
    answerString = formatNumber(expectedValue, 0);
  }
